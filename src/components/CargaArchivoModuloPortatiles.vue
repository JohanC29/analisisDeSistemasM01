...
<template>
  <div class="container">
    <div class="card">
      <div class="card-body">
        <h1 class="card-title">Subir archivo Usuarios CSV</h1>
        ...
        <div class="table-responsive">
          <h2 class="mt-4">Dependencias:</h2>
          <table class="table table-bordered">
            <thead class="table-light text-center">
              <th v-for="key in listDependencias" :key="key">
                {{ key }}
              </th>
            </thead>
            <tbody class="text-center">
              <tr v-for="(item, index) in listDependenciasData" :key="index">
                <td v-for="key in listDependencias" :key="key">
                  {{ item[key] }}
                </td>
              </tr>
            </tbody>
...
export default {
  computed: {
    ...
    listDependencias() {
      return this.listDependenciasData.length > 0
        ? Object.keys(this.listDependenciasData[0])
        : [];
    },
  },
  mounted() {
  ...
  data() {
    ...
    return {
      ...
      listSemetresData: [],
      listJornadasData: [],
      listDependenciasData: [],
      listEstado: ["ACTIVO", "INACTIVO"],
      listRol: ["ESTUDIANTE", "PROFESOR", "FUNCIONARIO"],
    };
  },
  methods: {
    cargaDatosMaestros() {
      ...
      axios
        .get(GET_DEPENDENCIA)
        .then((response) => {
          this.listDependenciasData = response.data.list;
        })
        .catch((error) => {
          // Manejar el error
        });
    ...
    esValidoJornada(iidjornada) {
      return this.listJornadasData.some(function (elemento) {
        return parseInt(elemento.id) === iidjornada || iidjornada === null;
      });
    },
    ...
    esValidoDependencia(iiddependencia){
      return this.listDependenciasData.some(function (elemento) {
        return elemento === iiddependencia;
      });
    },
    procesarDatos() {
      console.log("Matriz");
